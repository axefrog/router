 ...
 * @param  {Stream<history.location>}               history$   Observable
 ...
 * for `.path()`
 */
function makePathFilter(history$, namespace) {
  /**
  ...
  return function path(pathname) {
    ...
    scopedHistory$.createHref = history$.createHref
    return createAPI(scopedHistory$, scopedNamespace)
  }
}
